...
#define PWR_CFG_NAME_SZ		32

static void siw_touch_do_parse_dts_pwr(struct siw_ts *ts,
				struct touch_pwr_cfg *cfg, char *pwr_name)
{
	struct device *dev = ts->dev;
	struct device_node *np = dev->of_node;
	char cfg_name[PWR_CFG_NAME_SZ] = { 0, };
	const char *name = NULL;
	int tmp;

	memset(cfg, 0, sizeof(struct touch_pwr_cfg));

	snprintf(cfg_name, PWR_CFG_NAME_SZ - 1, "%s-type", pwr_name);
	tmp = siw_touch_of_int(dev, np, cfg_name);
	if (tmp > 0) {
		cfg->type = tmp;
	} else {
		t_dev_dbg_base(dev, "%s not found\n", cfg_name);
	}

	switch (cfg->type) {
	case PWR_TYPE_REG:
		snprintf(cfg_name, PWR_CFG_NAME_SZ - 1, "%s-name", pwr_name);
		tmp = siw_touch_of_string(dev, np, cfg_name, &name);
		if (!tmp) {
			cfg->name = (char *)name;
		} else {
			t_dev_dbg_base(dev, "%s not found\n", cfg_name);
		...
		snprintf(cfg_name, PWR_CFG_NAME_SZ - 1, "%s-voltage", pwr_name);
		tmp = siw_touch_of_int(dev, np, cfg_name);
		if (tmp > 0) {
			cfg->voltage = tmp;
		} else {
		...
		snprintf(cfg_name, PWR_CFG_NAME_SZ - 1, "%s-load", pwr_name);
		tmp = siw_touch_of_int(dev, np, cfg_name);
		if (tmp > 0) {
			cfg->load = tmp;
		} else {
...
static int siw_touch_do_parse_dts(struct siw_ts *ts)
{
	...
	if (touch_fquirks(ts)->gpio_set_reset != NULL) {
		t_dev_info(dev, "reset pin ignored by gpio quirk\n");
		ts->flags |= TOUCH_SKIP_RESET_PIN;
	} else if (ts->flags & TOUCH_SKIP_RESET_PIN) {
	...
	siw_touch_do_parse_dts_pwr(ts, &pins->vdd_cfg, "vdd");
	siw_touch_do_parse_dts_pwr(ts, &pins->vio_cfg, "vio");
	...
	/* Role */
	tmp = siw_touch_of_int(dev, np, "use_cancel_opt");
	if (tmp >= 0) {
		role->use_cancel_opt = tmp;
	}
	tmp = siw_touch_of_int(dev, np, "use_palm_opt");
	if (tmp >= 0) {
		role->use_palm_opt = tmp;
	}
	...
	if (role->use_firmware == 1) {
		...
		tmp = siw_touch_of_int(dev, np, "use_fw_pwr_rst");
		if (tmp >= 0) {
			role->use_fw_pwr_rst = tmp;
		}

		tmp = siw_touch_of_int(dev, np, "use_fw_ver_diff");
		if (tmp >= 0) {
			role->use_fw_ver_diff = tmp;
		}

		tmp = siw_touch_of_int(dev, np, "use_fw_skip_pid");
		if (tmp >= 0) {
			role->use_fw_skip_pid = tmp;
		}
...
static int siw_touch_parse_dts(struct siw_ts *ts)
{
	...
	/* Role */
	role->use_cancel_opt = 0;
	role->use_palm_opt = 0;

	role->use_firmware = 0;
	role->use_fw_upgrade = 0;
	role->use_fw_pwr_rst = 0;
	role->use_fw_ver_diff = 0;
	role->use_fw_skip_pid = 0;
	ts->def_fwcnt = 0;
	memset(ts->def_fwpath, 0, sizeof(ts->def_fwpath));
	ts->panel_spec = NULL;
...
int siw_touch_parse_data(struct siw_ts *ts)
{
	...
	if (role->use_cancel_opt) {
		t_dev_info(dev, " use_cancel_opt  = 0x%X\n", role->use_cancel_opt);
	}
	if (role->use_palm_opt) {
		t_dev_info(dev, " use_palm_opt    = 0x%X\n", role->use_palm_opt);
	}
	t_dev_info(dev, " use_firmware    = %d\n", role->use_firmware);
	t_dev_info(dev, " use_fw_upgrade  = %d\n", role->use_fw_upgrade);
	if (role->use_fw_pwr_rst) {
		t_dev_info(dev, " use_fw_pwr_rst  = %d\n", role->use_fw_pwr_rst);
	}
	if (role->use_fw_ver_diff) {
		t_dev_info(dev, " use_fw_ver_diff = %d\n", role->use_fw_ver_diff);
	}
	if (role->use_fw_skip_pid) {
		t_dev_info(dev, " use_fw_skip_pid = %d\n", role->use_fw_skip_pid);
	}
