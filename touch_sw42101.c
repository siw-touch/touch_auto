...
static int __used sw42101_init_pre(struct device *dev)
{
...
//	struct siw_ts *ts = chip->ts;
	...
	if (ret < 0) {
		t_dev_err(dev, "init pre for SW42101 failed, %d\n", ret);
		return ret;
	}

	t_dev_info(dev, "init pre for SW42101: %s\n", temp);
...
enum {
	__BIN_VER_OFFSET_POS = (CHIP_FW_SIZE - 0x14),			//0x0001_DFEC
	__BIN_PID_OFFSET_POS = (CHIP_FW_SIZE - 0x10),			//0x0001_DFF0
};
...
static int __used sw42101_fwup_check(struct device *dev, u8 *fw_buf)
{
	struct siw_touch_chip *chip = to_touch_chip(dev);
	struct siw_ts *ts = chip->ts;
	struct siw_hal_fw_info *fw = &chip->fw;
	struct siw_hal_tc_version *bin_ver;
	u32 bin_ver_offset = __BIN_VER_OFFSET_POS;
	u32 bin_pid_offset = __BIN_PID_OFFSET_POS;
	u32 dev_major = 0;
	u32 dev_minor = 0;
	char pid[12] = {0, };
	u32 bin_major = 0;
...
//	int ret = 0;
	...
	if (ts->force_fwup) {
		...
		if (use_fw_ver_diff) {
			u32 bin_ver = (bin_major<<16) | (bin_minor<<8);
			u32 dev_ver = (dev_major<<16) | (dev_minor<<8);
			if (bin_ver != dev_ver) {
				update |= BIT(8);
			}
		} else {
			update |= siw_hal_fw_ver_cmp(bin_major, bin_minor, 0,
	...
	if (fw->invalid_pid) {
		t_dev_warn(dev,
			"FW compare: dev-pid[%s] is invalid, skip pid check\n",
			fw->product_id);
		goto out;
	}
#endif
...
static int sw42101_fwup_down(struct device *dev, u8 *fw_buf, int fw_size)
{
	...
	while (fw_size) {
		...
		if (!fw_size || !(fw_dn_size & (FW_WR_LOG_UNIT-1))) {
...
#define CHIP_INIT_PRE		sw42101_init_pre
#define CHIP_FWUP_CHECK		sw42101_fwup_check
#define CHIP_FWUP_UPGRADE	sw42101_fwup_upgrade
...
static const struct siw_touch_pdata chip_pdata = {
	...
	//function quirks
	.fquirks = {
		.init_pre		= CHIP_INIT_PRE,
		.fwup_check		= CHIP_FWUP_CHECK,
		.fwup_upgrade	= CHIP_FWUP_UPGRADE,
	},
};
